...
/*   Updated: 2023/01/06 15:03:54 by rschlott         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&philo_struct \
...
static void	think_routine(t_philo *philo_struct, bool tf)
{
	...
	time_to_think = (philo_struct->table->time_to_die - (time_in_ms()
				- philo_struct->last_meal) - philo_struct->table->time_to_eat)
		/ 2;
...
philos with uneven id-number have a delay of 2ms - cause a small delay -of their 
meal time. This way even-id philos eat first and deadlock is prevented */
